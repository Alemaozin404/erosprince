...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Inicializar tema claro/escuro da página
    initPageTheme();
...
function initPageTheme() {
    ...
    if (savedPageTheme) {
        document.documentElement.setAttribute('data-page-theme', savedPageTheme);
        App.state.isDarkTheme = savedPageTheme === 'dark';
    } else {
        App.state.isDarkTheme = true;
    ...
    toggle.addEventListener('click', () => {
        ...
        const themeMode = App.state.isDarkTheme ? 'dark' : 'light';
        document.documentElement.setAttribute('data-page-theme', themeMode);
        localStorage.setItem('pageTheme', themeMode);
        
        // Atualizar ícone
        const icon = toggle.querySelector('i');
        icon.className = App.state.isDarkTheme ? 'fas fa-moon' : 'fas fa-sun';
        
        showToast(App.state.isDarkTheme ? 'Modo escuro ativado! 🌙' : 'Modo claro ativado! ☀️', 'success');
    });
}
...
function generateShareableLink(formData) {
    ...
    if (formData.audio) params.set('audio', formData.audio);
    if (formData.photo) params.set('photo', formData.photo);
...
function setupDragDrop() {
    ...
    audioInput.addEventListener('change', (e) => {
        if (e.target.files[0]) processFiles(e.target.files, 'audio');
    });

    photoInput.addEventListener('change', (e) => {
        if (e.target.files[0]) processFiles(e.target.files, 'photo');
    });
}
...
function removeFile(type) {
    ...
    if (type === 'audio' && items.length > 0) {
        ...
        items.forEach(item => {
            if (item.textContent.includes('📷')) item.remove();
        });
    }
...
function parseCelebrationData(params) {
    return {
        ...
        likes: [params.get('like1'), params.get('like2'), params.get('like3')].filter(Boolean)
    };
}
...
function showCelebrationDisplay(data) {
    ...
    if (data.recipient) document.getElementById('recipient-text').textContent = 'Para: ' + escapeHtml(data.recipient);
    if (data.sender) document.getElementById('sender-text').textContent = 'De: ' + escapeHtml(data.sender);
    document.getElementById('message-text').textContent = data.message;

    if (data.date) {
        const dateElement = document.getElementById('date-text');
        const formattedDate = new Date(data.date).toLocaleDateString('pt-BR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
        dateElement.textContent = '📅 ' + capitalizeFirst(formattedDate);
        dateElement.style.display = 'inline-block';
    }

    if (data.link) {
        document.getElementById('link-text').innerHTML = '<a href="' + escapeHtml(data.link) + '" target="_blank" rel="noopener noreferrer"><i class="fas fa-external-link-alt"></i> ' + truncateUrl(data.link) + '</a>';
    }

    if (data.likes && data.likes.length > 0) {
        document.getElementById('likes-text').innerHTML = '<h3>Coisas que eu gosto em você</h3><ul>' + data.likes.map(like => '<li>' + escapeHtml(like) + '</li>').join('') + '</ul>';
    }
    ...
    if (data.youtube) {
        ...
        if (videoId) embedYouTubePlayer(videoId);
    }

    if (data.audio) embedAudioPlayer(data.audio);
...
function embedAudioPlayer(audioBase64) {
    ...
    playerContainer.style.display = 'block';
    document.getElementById('celebration-audio').play().catch(() => {});
}
...
function loadLikesAndViews() {
    ...
    if (localStorage.getItem('hasLiked_' + id)) {
...
function generateQRCode() {
    ...
    qrDiv.innerHTML = '';
    new QRCode(qrDiv, { text: linkInput.value, width: 200, height: 200, colorDark: '#1a1a2e', colorLight: '#ffffff', correctLevel: QRCode.CorrectLevel.H });
    qrContainer.style.display = 'flex';
    qrContainer.addEventListener('click', (e) => {
        if (e.target === qrContainer) closeQRCode();
    });
}
...
function shareCelebration() {
    if (navigator.share) {
        navigator.share({ title: 'Celebração Especial', text: 'Olha que mensagem linda!', url: window.location.href });
    } else {
        copyLink();
    }
}
